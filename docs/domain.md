...
2. A mensagem é persistida com o status `enviado` (`SENT`).
3. Algum consumidor processa a mensagem e atualiza seu status para `recebido` (`RECEIVED`).
4. Um consumidor lê a mensagem, e o status é atualizado para `lido` (`READ`).
...
- O status `enviado` (`SENT`) é o estado inicial de uma mensagem.
- O status `recebido` (`RECEIVED`) indica que a mensagem foi processada, mas ainda não lida.
- O status `lido` (`READ`) indica que a mensagem foi lida por um consumidor.
